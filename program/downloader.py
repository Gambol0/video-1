...
@Client.on_message(command(["اغنيه", f"song@{bn}"]) & ~filters.edited)
def song(_, message):
...
@Client.on_message(
    command(["تحميل", f"vsong@{bn}", "video", f"video@{bn}"]) & ~filters.edited
)
...
@Client.on_message(command(["كلمات", f"lyric@{bn}", "lyrics"]))
